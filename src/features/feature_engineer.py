...
class FeatureEngineer:
    """Handles feature transformation for training and inference."""
    ...
    def encode(self, user_id: str, item_id: str, category: str) -> Tuple[int, int, int]:
        """Encode string IDs to integer indices."""
        return (
            self.user_vocab.get(user_id, 1),
            self.item_vocab.get(item_id, 1),
            self.category_vocab.get(category, 1),
        )

    def build_user_history_tensor(
        self, history_ids: List[int]
    ) -> Tuple[np.ndarray, np.ndarray]:
        """Pad/truncate user history to max_seq_len."""
    ...
    def prepare_features(
        ...
        item_ids: List[str],
        categories: List[str],
        user_histories: Optional[Dict[str, List[int]]] = None,
        item_features: Optional[np.ndarray] = None,
    ) -> dict:
        """Prepare feature dict for batch inference.
