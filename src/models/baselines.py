...
class PopularModel:
    ...
    def recommend(self, k: int = 20, exclude: List[str] = None) -> List[str]:
...
class ItemCF:
    ...
    def fit(self, df: pd.DataFrame) -> "ItemCF":
    ...
    def _build_similarity(self, df: pd.DataFrame) -> None:
        n_items = len(self.item_ids)
        if n_items > 5000:
            # Skip full similarity for large item sets
            return
        ...
        for i, item_i in enumerate(self.item_ids):
            ...
            self.item_sim[item_i] = {
                self.item_ids[j]: float(row[j])
                for j in top_idx
                if j != i and row[j] > 0
            }

    def recommend(self, user_items: List[str], k: int = 20) -> List[str]:
...
class UserCF:
    ...
    def fit(self, df: pd.DataFrame) -> "UserCF":
        self.user_items = df.groupby("user_id")["item_id"].apply(list).to_dict()
        self.item_users = df.groupby("item_id")["user_id"].apply(list).to_dict()
        return self

    def recommend(self, user_id: str, user_items: List[str], k: int = 20) -> List[str]:
