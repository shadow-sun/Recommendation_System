...
def get_event_queue(cfg=None):
    """Return the global event queue, creating it on first call.

    When the project config has ``kafka.enabled = True``, a Kafka-backed
    queue is returned.  Otherwise the in-memory ``EventQueue`` is used.
    """
    global _event_queue
    ...
    with _lock:
        ...
        try:
            from .kafka_bridge import create_event_queue
